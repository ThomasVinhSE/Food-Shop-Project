        ...
        try
        {
            ...
            if(session != null)
            {
                ...
                if(cart != null)
                {
                    ...
                    if(items != null)
                    {
                        Map<TblProductDTO,Integer> list = null;
                        TblProductDAO dao = new TblProductDAO();
                        Set<Integer> id = items.keySet();
                        for (Integer integer : id) {
                            int quantity  = items.get(integer);
                            TblProductDTO dto = dao.getItemById(integer);
                            if(list == null)
                                list = new HashMap<>();
                            if(dto != null)
                                list.put(dto, quantity);
                        }
            ...
            log("Naming_ViewCartServlet: "+ex.getMessage());
        } catch (SQLException ex) {
            log("SQL_ViewCartServlet: "+ex.getMessage());
        }        finally{
